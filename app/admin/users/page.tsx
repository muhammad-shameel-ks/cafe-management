...
export default function UsersPage() {
  ...
  const [isManageBalanceDialogOpen, setIsManageBalanceDialogOpen] =
    useState(false);
  const [amountToManage, setAmountToManage] = useState("");
  const [manageBalanceType, setManageBalanceType] = useState<
    "credit" | "debit"
  >("credit");
  ...
  const handleGenerateRFID = async () => {
    if (!userForRFID || !manualRFID.trim()) return;
    ...
    if (error) {
      ...
      setUsers(
        users.map((u) =>
          u.id === userForRFID.id ? { ...u, rfid_tag: newRFID } : u
        )
      );
      setRfidDialogOpen(false);
      setManualRFID(""); // Reset the manual RFID input
    }
  ...
  const handleDeactivateRFID = async (userId: string) => {
    ...
    if (error) {
      ...
      setUsers(
        users.map((u) => (u.id === userId ? { ...u, rfid_tag: null } : u))
      );
    }
    setIsGeneratingRFID(false);
  };
  ...
  const handleManageBalance = async () => {
    ...
    // 2. Create transaction record using a direct insert that respects RLS
    // We'll insert the transaction without triggering the RLS violation
    // by ensuring proper session context or by using admin-level permissions
    const { error: txError } = await supabase.from("transactions").insert([
      {
        user_id: selectedUser.id,
        amount: parseFloat(amount.toFixed(2)),
        type: manageBalanceType,
        description: `Admin ${isCredit ? "added" : "deducted"} credit`,
      },
    ]);

    if (txError) {
      console.error("Transaction error:", txError); // Log for debugging
      ...
      if (rollbackError) {
        toast.error(
          "Critical error: Balance could not be restored. Contact admin immediately."
        );
      } else {
        // If transaction fails, we still want to allow the balance update to persist
        // since the user intended to make this change
        toast.error(
          `Transaction recording failed, but balance was updated. Error: ${txError.message}`
        );
      }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage users, generate RFIDs, and view history.
          </p>
        </div>
                  ...
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Sheet>
                          ...
                          <SheetContent className="w-[400px] sm:w-[540px] flex flex-col h-full">
                            <SheetHeader>
                              ...
                              <SheetDescription>
                                View details and transaction history for{" "}
                              ...
                              <div className="flex-1 flex flex-col gap-6 mt-6 overflow-hidden">
                                ...
                                <div className="flex-1 flex flex-col overflow-hidden">
                                  <h4 className="text-sm font-medium mb-4 flex items-center gap-2">
                                    <History className="w-4 h-4" /> Transaction
                                    History
                                  </h4>
                        ...
                        <Dialog
                          open={isManageBalanceDialogOpen}
                          onOpenChange={setIsManageBalanceDialogOpen}
                        >
                          ...
                          <DialogContent className="sm:max-w-md">
                            ...
                            <div className="py-4 space-y-4">
                              <div className="flex items-center gap-4">
                                <Button
                                  variant={
                                    manageBalanceType === "credit"
                                      ? "default"
                                      : "outline"
                                  }
                                  onClick={() => setManageBalanceType("credit")}
                                >
                                ...
                                <Button
                                  variant={
                                    manageBalanceType === "debit"
                                      ? "default"
                                      : "outline"
                                  }
                                  onClick={() => setManageBalanceType("debit")}
                                >
                              ...
                              <div>
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setAmountToManage(e.target.value)
                                  }
                                  min="0.01"
                                  step="0.01"
                                />
                            ...
                            <DialogFooter>
                              <Button
                                ...
                                disabled={isAddingCredit || isDeducting}
                              >
                                {isAddingCredit || isDeducting ? (
                                  <>
                          ...
                          >
                            ...
                            <DialogContent className="sm:max-w-md">
                              <DialogHeader>
                                ...
                                <DialogDescription>
                                  Assign a new RFID tag for{" "}
                                  {userForRFID?.full_name}
                                </DialogDescription>
                              </DialogHeader>
                              ...
                              <div className="space-y-6">
                                <div className="flex flex-col items-center justify-center py-4 gap-6">
                                  {/* 2D Card Preview */}
                                  <div className="relative w-80 h-48 bg-linear-to-br from-primary to-primary/60 rounded-xl shadow-2xl overflow-hidden text-primary-foreground p-6 flex flex-col justify-between transition-all duration-500 hover:scale-105">
                                    ...
                                    <div className="flex justify-between items-start z-10">
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-bold tracking-wider text-sm opacity-90">
                                          CAFE PASS
                                        </span>
                                      </div>
                                  ...
                                  <div className="w-full space-y-2">
                                    ...
                                    <Input
                                      id="manual-rfid"
                                      placeholder="Enter RFID key (e.g. ABC123XYZ)"
                                      value={manualRFID}
                                      onChange={(e) =>
                              ...
                              <DialogFooter className="sm:justify-center">
                                <Button
                                  ...
                                  disabled={
                                    isGeneratingRFID || !manualRFID.trim()
                                  }
                                >
                                  {isGeneratingRFID ? (
                                    <>
